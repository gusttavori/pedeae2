    ...
    try {
        ...
        if (newStatus === 'novo') {
            ...
            if (tokens.length > 0) {
                ...
                const message = {
                    notification: { // App Mostra na Tela Celular Nativo
        ...
        // Maquina Reacoes Parte 2: Se A cozinha Mudou Status No Painel KDS Pra (Ta fazneo, Saiu p etrega) Matcher Transitions Switch Logic Routing Routing Notifications Switches 
            ...
            if (userSnap.exists) {
                ...
                const validTokens = [...new Set(userTokens.filter((t: any) => typeof t === 'string' && t.length > 0))] as string[]; // Validations Check Empty String String Maps Types 
